...
- `SonarMark-Svr-QualityGate` — `QualityGateStatus` and `Conditions` hold the quality gate data
- `SonarMark-Svr-Issues` — `Issues` holds the list of fetched issues
- `SonarMark-Svr-HotSpots` — `HotSpots` holds the list of fetched hot-spots
- `SonarMark-Rpt-Markdown` — `ToMarkdown` generates the markdown report content
- `SonarMark-Rpt-Depth` — the `depth` parameter controls heading levels
- `SonarMark-Rpt-QualityGate` — quality gate status and conditions are included in the report
- `SonarMark-Rpt-Issues` — issues are categorized and rendered in the report
- `SonarMark-Rpt-HotSpots` — hot-spots are rendered in the report
